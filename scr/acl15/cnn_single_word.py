...
if __name__ == "__main__":
    ...
    for s in word_senses["bank"]:
        for s1 in word_senses["bank"]:
            print "sense 1",s, "sense 2",s1
            if ( word_sense_vectors.has_key(s) ):
                s1_vec = word_sense_vectors[s]
            else:
                s1_vec = word_vectors["bank"]
            if ( word_sense_vectors.has_key(s1) ):
                s2_vec = word_sense_vectors[s1]
            else:
                s2_vec = word_vectors["bank"]
            print "their distance", np.linalg.norm( s1_vec - s2_vec )
